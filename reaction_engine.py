...
def klasifikasikan_jenis_reaksi(persamaan):
    reaksi = persamaan.replace("\\rightarrow", "->").replace(" ", "")
    reaktan, produk = reaksi.split("->")
    jumlah_reaktan = len(reaktan.split("+"))
    jumlah_produk = len(produk.split("+"))

    if "CH" in reaktan and "O2" in reaktan and "CO2" in produk and "H2O" in produk:
        return "Reaksi pembakaran"
    elif jumlah_reaktan == 2 and jumlah_produk == 1:
        return "Reaksi sintesis (kombinasi)"
    elif jumlah_reaktan == 1 and jumlah_produk == 2:
        return "Reaksi dekomposisi"
    elif jumlah_reaktan == 2 and jumlah_produk == 2:
        return "Reaksi substitusi ganda"
    elif jumlah_reaktan == 2 and jumlah_produk == 2 and ("H2" in reaktan or "Cl2" in reaktan):
        return "Reaksi substitusi tunggal"
    elif any(x in produk for x in ["H2O"]) and any(x in reaktan for x in ["HCl", "NaOH", "KOH"]):
        return "Reaksi asam-basa (netralisasi)"
    elif any(x in produk for x in ["O2", "CO2", "H2O"]) and any(x in reaktan for x in ["O2", "H2"]):
        return "Reaksi redoks"
    else:
        return "Jenis reaksi belum dikenali"
